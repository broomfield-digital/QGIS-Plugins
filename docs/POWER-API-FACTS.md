...
| **`QDateTime(QDate)` no longer exists.** Qt6 dropped the single-argument overload; a `QTime` is mandatory. It fails at *plugin load*, inside `initAlgorithm`, so the symptom is "the whole provider vanished" rather than a bad default | measured | `processing/base.py` passes `QTime(0, 0)` |
| **`QDateTime.toPyDateTime()` returns a naive datetime** even for a UTC-stamped range | So comparing it against timezone-aware series timestamps is a `TypeError` — and inside a `updateTemporalRange` slot that means the handler dies every time the user moves the time slider | `gui/plot_widget.set_cursor` normalises to UTC before comparing |
| A Python UTC `datetime` becomes a **LocalTime** `QDateTime` | So `offsetFromUtc() == 0` is satisfied on a UTC machine even when the timezone was never set — which makes a naive-vs-aware bug invisible in UTC-based CI. Assert `timeSpec()` and `timeZone().id()`, which are zone-invariant | `_qdatetime()` in `layers_point` and `layers_raster` sets the zone explicitly |
| `QgsProcessingParameterEnum` defaults to **integer indices** on the command line | So `TEMPORAL=1` means different things to the point and gridded algorithms, whose option lists differ (gridded has no hourly) | `usesStaticStrings=True` + `parameterAsEnumString`, so the CLI reads `TEMPORAL=daily` |
